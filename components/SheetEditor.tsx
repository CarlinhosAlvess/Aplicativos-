...
import { SaveIcon, TableIcon, CloudIcon, CodeIcon, AlertIcon, SearchIcon, RefreshCwIcon, CalendarIcon, UsersIcon } from './Icons';
...
const SheetEditor = ({ onCloudConfig, isCloudConfigured, isSyncing, currentUser }: SheetEditorProps) => {
  ...
  // ESTILO DAS ABAS ALTERADO PARA LATERAL - COMPRESSED
  const tabClass = (tab: string) => `w-full text-left px-3 py-2 text-[10px] sm:text-xs font-bold transition-all border-l-4 ${
    activeTab === tab 
      ? 'bg-white text-indigo-600 border-indigo-600 shadow-[2px_0_10px_rgba(0,0,0,0.02)]' 
      : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-100/80'
  }`;
  ...
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden flex flex-col h-[calc(100vh-140px)]">
                      ...
                      <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-emerald-700 mt-1 leading-relaxed">
                              O App salva os dados automaticamente na nuvem...
                          </p>
                        </div>
                      </div>
                       <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                          <label className="block text-xs font-bold text-indigo-800 mb-1 uppercase tracking-wide">Endpoint</label>
      ...
      {/* Header da Planilha - COMPACT */}
      <div className="bg-slate-50 border-b border-slate-200 p-2 sm:p-3 flex justify-between items-center flex-none">
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="bg-white p-1.5 rounded-lg border border-slate-200 shadow-sm"><TableIcon className="w-4 h-4 text-indigo-600" /></div>
          <span className="font-bold text-slate-700 text-xs sm:text-sm">Editor de Dados</span>
        </div>
        
        <div className="flex items-center gap-1 sm:gap-2">
            <button 
                onClick={() => setShowApiModal(true)}
                className="text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 px-2 py-1.5 rounded-lg transition-all flex items-center gap-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-wider"
            ...
            <button 
                ...
                className={`text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 px-2 py-1.5 rounded-lg transition-all flex items-center gap-1.5 text-[10px] sm:text-xs font-bold uppercase tracking-wider ${isCloudConfigured ? 'text-emerald-500 hover:text-emerald-600 hover:bg-emerald-50' : ''}`}
                title="Configurar Nuvem"
            >
                <CloudIcon className={`w-3 h-3 sm:w-4 sm:h-4 ${isSyncing ? 'animate-spin' : ''}`} />
                <span className="hidden sm:inline">{isCloudConfigured ? 'Conectado' : 'Conectar'}</span>
            </button>

            <button 
                onClick={handleSave}
                className="px-3 sm:px-4 py-1.5 rounded-lg font-bold text-[10px] sm:text-xs transition-all shadow-md flex items-center gap-1.5 ml-1 bg-indigo-600 hover:bg-indigo-700 text-white shadow-indigo-100"
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          {/* SIDEBAR TABS - COMPRESSED */}
          <div className="w-32 sm:w-40 bg-slate-50 border-r border-slate-200 flex flex-col overflow-y-auto flex-none py-2">
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-white">
            ...
            {/* FILTER BAR - COMPACT */}
            <div className="bg-white px-2 sm:px-4 py-2 border-b border-slate-100 flex-none">
                <div className="relative">
                    <input 
                        type="text" 
                        ...
                        className="w-full pl-8 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all"
                    />
            ...
            <div className="flex-1 overflow-hidden p-2 sm:p-4 flex flex-col">
                ...
                <div className="space-y-4 h-full overflow-y-auto pr-2 custom-scrollbar">
                    <div className="bg-amber-50 border border-amber-100 rounded-lg p-2 flex gap-2">
                        ...
                        <p className="text-[10px] sm:text-xs text-amber-800 leading-relaxed">
                            <strong>Vagas:</strong> Defina o total de vagas por turno.
                        </p>
                    </div>
                    ...
                    <thead>
                        ...
                        <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 rounded-tl-lg shadow-sm">Nome</th>
                        <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 min-w-[200px] shadow-sm">Área de Atuação</th>
                        <th className="sticky top-0 z-20 bg-indigo-50/50 p-2 font-bold text-slate-600 text-center w-16 shadow-sm">Manhã</th>
                        <th className="sticky top-0 z-20 bg-indigo-50/50 p-2 font-bold text-slate-600 text-center w-16 shadow-sm">Tarde</th>
                        <th className="sticky top-0 z-20 bg-indigo-50/50 p-2 font-bold text-slate-600 text-center w-16 shadow-sm">18h</th>
                        <th className="sticky top-0 z-20 bg-amber-50/50 p-2 font-bold text-slate-600 text-center w-16 shadow-sm">Sáb</th>
                        <th className="sticky top-0 z-20 bg-rose-50/50 p-2 font-bold text-slate-600 text-center w-16 shadow-sm">Dom</th>
                        <th className="sticky top-0 z-20 bg-purple-50/50 p-2 font-bold text-slate-600 text-center w-16 rounded-tr-lg shadow-sm">Feriado</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        ...
                        <tr key={tech.id} className="hover:bg-white hover:shadow-md hover:scale-[1.01] transition-all duration-200 group relative z-0 hover:z-10 border-b border-transparent hover:border-slate-100 rounded-lg">
                            <td className="p-1.5">
                            <input 
                                className="w-full p-1.5 rounded-lg border-transparent hover:border-slate-200 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 outline-none bg-transparent transition-all font-medium text-slate-700 text-xs"
                                value={tech.nome}
                            ...
                            <td className="p-1.5">
                                <div className="flex flex-wrap gap-1">
                                    {tech.cidades.map((city, cIdx) => (
                                        <span key={cIdx} className="bg-white border border-slate-200 text-slate-600 px-1.5 py-0.5 rounded text-[10px] font-medium flex items-center gap-1 shadow-sm">
                                            {city}
                                    ...
                                    {newCityInput?.techIndex === idx ? (
                                        <select 
                                            autoFocus
                                            className="border border-indigo-300 rounded px-1 py-0.5 text-[10px] w-24 outline-none shadow-sm"
                                        ...
                                        <button onClick={() => setNewCityInput({techIndex: idx, value: ''})} className="text-indigo-600 hover:bg-indigo-50 px-1.5 py-0.5 rounded text-[10px] font-bold transition-colors">+</button>
                                    )}
                                </div>
                            </td>
                            <td className="p-1 bg-indigo-50/30 group-hover:bg-indigo-50/50 transition-colors"><input type="number" step="1" min="0" className="w-full p-1.5 text-center outline-none bg-transparent font-medium text-slate-600 text-xs" value={tech.capacidadeManha} onChange={(e) => handleTechChange(idx, 'capacidadeManha', e.target.value)} /></td>
                            <td className="p-1 bg-indigo-50/30 group-hover:bg-indigo-50/50 transition-colors"><input type="number" step="1" min="0" className="w-full p-1.5 text-center outline-none bg-transparent font-medium text-slate-600 text-xs" value={tech.capacidadeTarde} onChange={(e) => handleTechChange(idx, 'capacidadeTarde', e.target.value)} /></td>
                            <td className="p-1 bg-indigo-50/30 group-hover:bg-indigo-50/50 transition-colors"><input type="number" step="1" min="0" className="w-full p-1.5 text-center outline-none bg-transparent font-medium text-slate-600 text-xs" value={tech.capacidadeNoite || 0} onChange={(e) => handleTechChange(idx, 'capacidadeNoite', e.target.value)} /></td>
                            <td className="p-1 bg-amber-50/30 group-hover:bg-amber-50/50 transition-colors"><input type="number" step="1" min="0" className="w-full p-1.5 text-center outline-none bg-transparent font-bold text-amber-700 text-xs" value={tech.capacidadeSabado || 0} onChange={(e) => handleTechChange(idx, 'capacidadeSabado', e.target.value)} /></td>
                            <td className="p-1 bg-rose-50/30 group-hover:bg-rose-50/50 transition-colors"><input type="number" step="1" min="0" className="w-full p-1.5 text-center outline-none bg-transparent font-bold text-rose-700 text-xs" value={tech.capacidadeDomingo || 0} onChange={(e) => handleTechChange(idx, 'capacidadeDomingo', e.target.value)} /></td>
                            <td className="p-1 bg-purple-50/30 group-hover:bg-purple-50/50 transition-colors"><input type="number" step="1" min="0" className="w-full p-1.5 text-center outline-none bg-transparent font-bold text-purple-700 text-xs" value={tech.capacidadeFeriado || 0} onChange={(e) => handleTechChange(idx, 'capacidadeFeriado', e.target.value)} /></td>
                        </tr>
                        )})}
                    </tbody>
                    </table>
                    </div>
                    <button onClick={handleAddTech} className="text-indigo-600 hover:text-indigo-800 font-bold text-xs bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors shadow-sm">+ Novo Técnico</button>
                </div>
                )}
                ...
                {['cidades', 'atividades', 'feriados'].includes(activeTab) && (
                    <div className="space-y-4 h-full overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        <table className="w-full border-collapse text-xs max-w-lg">
                            <thead>
                                <tr className="bg-slate-50 text-left border-b border-slate-100 shadow-sm">
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 rounded-tl-lg shadow-sm">
                                        {activeTab === 'feriados' ? 'Data' : 'Nome / Valor'}
                                    </th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 w-10 rounded-tr-lg shadow-sm"></th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                    ...
                                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                        <td className="p-1.5">
                                            <input 
                                                ...
                                                className={`w-full p-1.5 rounded-lg border focus:ring-2 outline-none transition-all text-xs ${isInvalid ? 'border-rose-300 bg-rose-50' : 'border-transparent hover:border-slate-200 focus:border-indigo-300 focus:ring-indigo-100 bg-transparent'}`}
                                                value={item.value}
                                                onChange={(e) => {
                                        ...
                                        <td className="p-1.5 text-right">
                                            <button 
                                                ...
                                                className="text-slate-400 hover:text-rose-500 p-1.5 rounded-full hover:bg-rose-50 transition-all"
                                            >
                                                &times;
                                            </button>
                                        </td>
                        ...
                        <button 
                            ...
                            className="text-indigo-600 hover:text-indigo-800 font-bold text-xs bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors shadow-sm"
                        >
                            + Adicionar
                        </button>
                    </div>
                ...
                {activeTab === 'usuarios' && (
                    <div className="space-y-4 h-full overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        <table className="w-full border-collapse text-xs min-w-[600px]">
                            <thead>
                                <tr className="bg-slate-50 text-left border-b border-slate-100 shadow-sm">
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 rounded-tl-lg shadow-sm">Usuário</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Senha</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Perfil</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Permissões Específicas</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 w-10 rounded-tr-lg shadow-sm"></th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {filteredUsuarios.map((u, _) => {
                                    ...
                                    return (
                                    <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                        <td className="p-1.5"><input className="w-full p-1.5 bg-transparent border-transparent hover:border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-100 outline-none text-xs" value={u.nome} onChange={(e) => handleUsuarioNameChange(idx, e.target.value)} /></td>
                                        <td className="p-1.5"><input className="w-full p-1.5 bg-transparent border-transparent hover:border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-100 outline-none text-xs" type="text" value={u.senha} onChange={(e) => handleUsuarioPassChange(idx, e.target.value)} /></td>
                                        <td className="p-1.5">
                                            <select 
                                                className="w-full p-1.5 bg-transparent border-transparent hover:border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-100 outline-none cursor-pointer text-xs"
                                                value={u.perfil}
                                        ...
                                        <td className="p-1.5 text-right">
                                            <button onClick={() => handleDeleteUsuario(idx)} className="text-slate-400 hover:text-rose-500 p-1.5 rounded-full hover:bg-rose-50">&times;</button>
                                        </td>
                                    </tr>
                                )})}
                            </tbody>
                        ...
                        <button onClick={handleAddUsuario} className="text-indigo-600 hover:text-indigo-800 font-bold text-xs bg-indigo-50 px-3 py-1.5 rounded-lg transition-colors shadow-sm">+ Adicionar Usuário</button>
                    </div>
                )}
                
                {activeTab === 'logs' && (
                    <div className="overflow-x-auto h-full overflow-y-auto pr-2 custom-scrollbar">
                        <table className="w-full border-collapse text-xs whitespace-nowrap">
                            <thead>
                                <tr className="bg-slate-50 text-left border-b border-slate-100 shadow-sm">
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 rounded-tl-lg shadow-sm">Data/Hora</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Usuário</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Ação</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 rounded-tr-lg shadow-sm">Detalhes</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 font-mono text-[10px]">
                                {filteredLogs.map((log) => (
                                    <tr key={log.id} className="hover:bg-slate-50">
                                        <td className="p-1.5 text-slate-500">{new Date(log.timestamp).toLocaleString()}</td>
                                        <td className="p-1.5 font-bold text-slate-700">{log.usuario}</td>
                                        <td className="p-1.5">
                                        ...
                                        <td className="p-1.5 text-slate-600 max-w-xs truncate" title={log.detalhes}>{log.detalhes}</td>
                                    </tr>
                                ))}
                ...
                {activeTab === 'agendamentos' && (
                    <div className="space-y-4 h-full flex flex-col">
                        ...
                        <div className="flex flex-wrap gap-2 mb-2 bg-slate-50 p-2 rounded-lg border border-slate-200 flex-none">
                            <input 
                                type="date" 
                                value={filterDate} 
                                onChange={(e) => setFilterDate(e.target.value)} 
                                className="p-1.5 text-[10px] rounded border border-slate-300 outline-none focus:ring-2 focus:ring-indigo-100"
                                title="Filtrar por Data"
                            />
                            <select 
                                value={filterCity}
                                onChange={(e) => setFilterCity(e.target.value)}
                                className="p-1.5 text-[10px] rounded border border-slate-300 outline-none focus:ring-2 focus:ring-indigo-100 cursor-pointer"
                            >
                            ...
                            <button 
                                ...
                                className="px-2 py-1 text-[10px] font-bold text-slate-500 hover:text-slate-800 hover:bg-slate-200 rounded transition"
                            >
                                Limpar
                            </button>
                        </div>
                        ...
                        <table className="w-full border-collapse text-xs min-w-[1000px]">
                            <thead>
                                <tr className="bg-slate-50 text-left border-b border-slate-100 shadow-sm">
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 rounded-tl-lg shadow-sm">Data / Hora</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Cliente</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Cidade</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Técnico</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 text-center shadow-sm">Tipo</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Status Execução</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 font-bold text-slate-600 shadow-sm">Observação</th>
                                    <th className="sticky top-0 z-20 bg-slate-50 p-2 w-16 rounded-tr-lg text-center shadow-sm">Ações</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {filteredAgendamentos.map((ag) => {
                                    ...
                                    <tr key={ag.id} className={`hover:bg-slate-50 transition-colors ${isPre ? 'bg-amber-50/40' : (isIncidente ? 'bg-rose-50/40' : '')}`}>
                                        <td className="p-2 align-top">
                                            ...
                                            <div className="text-[10px] text-slate-500">{ag.periodo}</div>
                                            {isPre && ag.criadoEm && (
                                                <div className="mt-1 text-[10px] bg-amber-100 text-amber-800 px-1 py-0.5 rounded w-fit">
                                                    <CountdownTimer criadoEm={ag.criadoEm} />
                                                </div>
                                            )}
                                            {isIncidente && ag.dadosIncidente?.horarioAvistado && (
                                                <div className="mt-1 text-[10px] bg-rose-100 text-rose-800 px-1 py-0.5 rounded w-fit font-bold">
                                        ...
                                        <td className="p-2 align-top">
                                            ...
                                            <div className="text-[10px] text-slate-500">{ag.telefone}</div>
                                            <div className="text-[9px] text-indigo-500 font-bold mt-0.5 bg-indigo-50 inline-block px-1 rounded">{ag.atividade}</div>
                                            {isIncidente && ag.dadosIncidente?.endereco && (
                                                <div className="text-[9px] text-rose-600 mt-0.5 font-medium max-w-[150px] leading-tight">📍 {ag.dadosIncidente.endereco}</div>
                                            )}
                                        </td>
                                        <td className="p-2 align-top text-slate-600">{ag.cidade}</td>
                                        <td className="p-2 align-top">
                                            <div className="flex items-center gap-2">
                                                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${ag.tecnicoNome === 'Desconhecido' ? 'bg-slate-200 text-slate-500' : 'bg-indigo-100 text-indigo-600'}`}>
                                                    {ag.tecnicoNome.charAt(0)}
                                                </div>
                                                <span className="text-xs font-medium">{ag.tecnicoNome}</span>
                                            </div>
                                        </td>
                                        <td className="p-2 align-top text-center">
                                            {isPre ? (
                                                <span className="px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 text-[9px] font-bold border border-amber-200">Pré-Reserva</span>
                                            ) : isIncidente ? (
                                                <span className="px-1.5 py-0.5 rounded bg-rose-100 text-rose-700 text-[9px] font-bold border border-rose-200">Incidente</span>
                                            ) : (
                                                <span className="px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 text-[9px] font-bold border border-emerald-200">Confirmado</span>
                                            )}
                                        </td>
                                        <td className="p-2 align-top">
                                            {isPre ? (
                                                <button 
                                                    ...
                                                    className="w-full py-0.5 bg-amber-500 hover:bg-amber-600 text-white text-[9px] font-bold rounded shadow-sm transition-colors mb-1"
                                                >
                                                    Confirmar
                                                </button>
                                            ) : (
                                                <div className="space-y-1">
                                                    <select 
                                                        className={`w-full p-1 text-[10px] rounded border outline-none font-bold cursor-pointer ${
                                                    ...
                                                    {ag.statusExecucao === 'Não Finalizado' && (
                                                        <input 
                                                            type="text"
                                                            placeholder="Motivo..."
                                                            className="w-full p-1 text-[10px] border border-rose-300 rounded bg-rose-50 text-rose-800 placeholder-rose-300 outline-none focus:ring-1 focus:ring-rose-500"
                                        ...
                                        <td className="p-2 align-top">
                                            <textarea 
                                                className="w-full p-1 text-[10px] bg-slate-50 border-transparent hover:border-slate-200 rounded focus:bg-white focus:border-indigo-300 outline-none resize-none h-12 transition-all"
                                                placeholder="..."
                                                value={ag.observacao || ''}
                                                onChange={(e) => handleObservacaoChange(idx, e.target.value)}
                                            />
                                            {isIncidente && ag.dadosIncidente?.descricaoTecnico && (
                                                <div className="mt-0.5 text-[9px] text-slate-500 bg-slate-100 p-0.5 rounded italic truncate max-w-[150px]" title={ag.dadosIncidente.descricaoTecnico}>
                                                    "{ag.dadosIncidente.descricaoTecnico}"
                                                </div>
                                            )}
                                        </td>
                                        <td className="p-2 align-top text-center space-y-1">
                                            <button 
                                                onClick={() => openRescheduleModal(ag)}
                                                className="p-1 text-amber-500 hover:text-amber-700 hover:bg-amber-50 rounded transition-colors w-full flex justify-center"
                                                title="Reagendar"
                                            >
                                            ...
                                            <button 
                                                ...
                                                className="p-1 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded transition-colors w-full flex justify-center"
                                                title="Excluir"
                                            >
                                                &times;
                                            </button>
